...
mat read_header(std::istream& is, uint32_t& starting_point, uint32_t& num_colors) {
    ...
    if (num_colors == 0) num_colors = pow(2, 4);
    for (uint8_t i=0; i<4; ++i) is.get();
    img.set_rows(rows);
    img.set_cols(cols);
    return img;
...
void read_pixels(std::istream& is, mat& img, std::vector<std::array<uint8_t, 4>>& colors) {
    uint8_t pad = (32-(img.get_cols()*4)%32)/8;
    while (is.peek() != EOF){
        for (uint32_t i=0; i<img.get_cols();++i) {
            uint8_t p = is.get();
            uint8_t p1 = p >> 4;
            img.add_pix(colors[p1][0],colors[p1][1], colors[p1][2]);
            if (i!=img.get_cols()-1) {
                uint8_t p2 = p & 0x0F;
                img.add_pix(colors[p2][0],colors[p2][1], colors[p2][2]);
                ++i;
            }
        }
